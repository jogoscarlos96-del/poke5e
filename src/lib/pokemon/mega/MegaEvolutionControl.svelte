<script lang="ts">
	...
	$: configuredActiveForm = MegaEvolution.activeForm(state)
	...
	$: isActive = configuredActiveForm != null && eligibility.eligible
	$: canEdit = $trainer.update != null && $trainer.writeKey != null
	$: canMegaEvolve = canEdit && state.forms.length > 0 && eligibility.eligible

	$: unavailableReason = state.forms.length === 0
		? "Configure a Mega form in Edit Pokémon first."
...
<div class:active={isActive} class:invalid-active={configuredActiveForm != null && !isActive} class="mega-control">
	<div class="heading">
		<div>
			<strong>{isActive ? configuredActiveForm?.name : "Mega Evolution"}</strong>
			{#if isActive}<span class="badge">MEGA</span>{/if}
		</div>
		{#if configuredActiveForm != null}
			<Button on:click={() => setActive(null)} disabled={saving || !canEdit} variant="subtle">{saving ? "Reverting…" : "Revert"}</Button>
		{/if}
	</div>

	{#if isActive}
		<p class="rules">+2 AC · doubled ability modifiers for attacks, damage, saving throws, and save DCs{#if configuredActiveForm?.type} · {configuredActiveForm.type.toString()} type{/if}</p>
	{:else if configuredActiveForm != null && !eligibility.eligible}
		<p class="warning">The stored Mega form is not currently applied: {eligibility.reason}</p>
	...
	{#if configuredActiveForm == null && unavailableReason}<p class="hint">{unavailableReason}</p>{/if}
