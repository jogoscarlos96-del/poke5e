...
test("Move Roller resolves standard, combo, and repeated flows", async ({ page }) => {
	...
	await expect(attackFormula).toHaveText(baseAttackModifier >= 0 ? `+${baseAttackModifier + 1}` : `${baseAttackModifier + 1}`)
	...
	await expect(dialog.locator(".roll-formula strong")).toHaveText(baseAttackModifier >= 0 ? `+${baseAttackModifier}` : `${baseAttackModifier}`)
	await dialog.getByRole("button", { name: "Roll Attack", exact: true }).click()
	await dialog.getByRole("button", { name: "Hit", exact: true }).click()
	...
	await expect(damageFormula).toHaveText(baseDamageModifier >= 0 ? `+${baseDamageModifier + 1}` : `${baseDamageModifier + 1}`)
	await dialog.getByRole("button", { name: "Close", exact: true }).click()
	await expect(dialog).not.toBeVisible()
	...
	await expect(dialog.locator(".damage-roll .formula strong")).toHaveText(baseDamageModifier >= 0 ? `+${baseDamageModifier}` : `${baseDamageModifier}`)
